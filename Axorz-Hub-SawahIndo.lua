-- ============================================================
--  AXORZ HUB | v2.0 — GABUNGAN SEMUA TOGGLE
--  Basis: script v1.4 yang WORKS untuk chicken
--  Tambahan:
--  - GlobalPause flag agar semua loop berhenti saat chicken busy
--  - "Auto Farm All" aktif jika 4 toggle ON sekaligus:
--    AutoFarm + AutoFarmBesar + AutoFeed + AutoClaimEgg
--  - Chicken claim/feed TIDAK diubah dari versi lama (sudah works)
--  - Hanya koordinasi antar-loop yang diperbaiki
-- ============================================================
...
-- Chicken lock: pause semua loop farm saat chicken berjalan
...
local function ReleaseChickenLock()
    ...
    -- Un-anchor player agar bisa bergerak normal lagi
    local hrp = GetHRP()
    ...
    local function scan(parent)
        ...
        for _, item in pairs(parent:GetChildren()) do
            if item:IsA("Tool") then
                ...
                if not name:find("bibit") then continue end
...
DoFarmGabungan = function()
    ...
    local ok, err = pcall(function()
        ...
        for cropName, crop in pairs(OurCropsBesar) do
            if not crop or not crop.Parent then OurCropsBesar[cropName]=nil; continue end
            if not crop:GetAttribute("IsReady") then continue end
            pcall(function()
                for _, child in pairs(crop:GetDescendants()) do
                    if child:IsA("ProximityPrompt") and child.Enabled then
...
local function DetectLahanSaya()
    ...
    for _, entry in ipairs(sorted) do
        ...
        if not areaObj or not areaObj.Parent then continue end
...
-- ══════════════════════════════════════════════════════════
--  CHICKEN SYSTEM
--  !! DIAMBIL DARI VERSI LAMA YANG WORKS !!
--  Tidak ada perubahan pada logika claim/feed
-- ══════════════════════════════════════════════════════════
...
-- ── CONFIRM GUI HELPER (dari versi lama, sudah works) ──
...
-- ── AUTO FEED (dari versi lama, sudah works) ──
        ...
        for _, prompt in ipairs(feedPrompts) do
            ...
            if hrp then
                ...
                if slotPart and slotPart:IsA("BasePart") then
                    ...
                    -- Cari BasePart di parent hierarchy
                ...
                if targetPos then
                    -- Un-anchor sebentar untuk teleport, anchor lagi setelah
                    hrp.Anchored = false
                    task.wait(0.05)
                    hrp = GetHRP()
                    if hrp then
                        hrp.CFrame = CFrame.new(targetPos + Vector3.new(0, 3, 0))
                        hrp.Anchored = true -- anchor agar tidak drift saat tunggu ConfirmGui
                    end
                    task.wait(0.5)
...
-- ── AUTO CLAIM EGG ──
-- PENTING: EggVisual adalah [Part], prompt ada langsung sebagai child-nya
-- MaxActivationDistance = 10 studs → WAJIB teleport ke egg dulu
...
local function DoClaimEgg()
    ...
    local ok, err = pcall(function()
        ...
        for _, obj in pairs(coop:GetChildren()) do
            if not obj.Name:lower():find("egg") then continue end

            -- EggVisual adalah BasePart — prompt langsung sebagai child Part
            local prompt = nil
            local eggPos = nil

            if obj:IsA("BasePart") then
                -- Struktur: EggVisual [Part] → ProximityPrompt (child langsung)
                eggPos = obj.Position
                prompt = obj:FindFirstChildOfClass("ProximityPrompt")
                -- Fallback ke descendants
                if not prompt then
            ...
            -- TELEPORT ke posisi egg karena MaxDist = 10 studs
            if eggPos then
                -- Un-anchor sebentar untuk bisa teleport, lalu anchor lagi
                ...
                if hrp then
                    ...
                    hrp.Anchored = true -- anchor lagi agar tidak drift
                end
                task.wait(0.5) -- tunggu physics settle
...
--  GlobalPause ON → claim egg (dengan teleport per-egg) → feed → resume
-- ──────────────────────────────────────────────────────────
local function DoChickenRoutineForFarmAll()
    ...
    local ok, err = pcall(function()
        ...
        -- Claim egg — DoClaimEgg sudah teleport ke tiap egg (MaxDist=10)
        ClaimEggDebounce = false
        pcall(DoClaimEgg)
...
-- ── EGG WATCHER (dari versi lama, sudah works) ──
local function SetupEggWatcher(coop)
    ...
    EggWatcherConn = coop.ChildAdded:Connect(function(child)
        ...
        task.spawn(function()
            -- Tunggu prompt siap (EggVisual adalah Part, prompt sebagai child langsung)
            local prompt = nil
            for i = 1, 20 do
                task.wait(0.15)
            ...
            -- Teleport ke egg karena MaxDist = 10 studs
            local hrp = GetHRP()
            if hrp then
...
local function UpdateESP()
    ...
    for _, crop in pairs(ActiveCrops:GetChildren()) do
        if not crop or not crop.Parent then continue end
        local isOurs = OurCrops[crop.Name]~=nil or OurCropsBesar[crop.Name]~=nil
        if not Config.ESPShowAll and not isOurs then RemoveESPLabel(crop); continue end
...
-- ══════════════════════════════════════════════════════════
--  LOOPS — semua cek CanRun() agar berhenti saat chicken busy
-- ══════════════════════════════════════════════════════════
...
task.spawn(function()
    while true do
        ...
        if Config.IsFarmingBesar then continue end
...
task.spawn(function()
    while true do
        task.wait(3)
        if not CanRun() then continue end
        if not (Config.AutoHarvest or Config.AutoFarm) then continue end
        if Config.IsFarmingBesar then continue end
        for cropName, crop in pairs(OurCrops) do
            if not CanRun() then break end
            if not crop or not crop.Parent then OurCrops[cropName]=nil; continue end
            if crop:GetAttribute("IsReady") then
...
task.spawn(function()
    while true do
        task.wait(5)
        if not CanRun() then continue end
        if not (Config.AutoHarvestBesar or Config.AutoFarmBesar or Config.AutoFarm) then continue end
        local anyReady = false
        for cropName, crop in pairs(OurCropsBesar) do
            if not crop or not crop.Parent then OurCropsBesar[cropName]=nil; continue end
            if crop:GetAttribute("IsReady") then anyReady=true; break end
        end
        if not anyReady then continue end
        if Config.AutoFarm and Config.AutoFarmBesar then
            if not FarmGabunganDebounce and not Config.IsFarmingBesar and Config.ClaimedAreaBesar then
                FarmGabunganDebounce = true
                task.spawn(function() pcall(DoFarmGabungan) FarmGabunganDebounce=false end)
...
task.spawn(function()
    while true do
        task.wait(5)
        if not CanRun() then continue end
        if not (Config.AutoPlantBesar or Config.AutoFarmBesar) then continue end
        if #PlotPositionsBesar == 0 then continue end
        if GetOurCropBesarCount() >= MAX_BESAR then continue end
        if tick() - LastPlantBesarTime < PLANT_BESAR_COOLDOWN then continue end
...
-- Loop Auto Feed (hanya jika AutoFeed ON dan AutoClaimEgg OFF)
-- Jika keduanya ON, watcher yang handle via DoChickenRoutineForFarmAll
task.spawn(function()
    while true do
        task.wait(10)
        if not Config.AutoFeed then continue end
        if Config.IsChickenBusy then continue end
        -- Jika AutoClaimEgg juga ON, biarkan watcher yang punya kendali
        -- Tapi tetap polling sebagai backup (setelah 10s tidak ada trigger)
        pcall(DoFeedChicken)
    end
end)

-- Loop Auto Claim Egg backup (polling)
task.spawn(function()
    while true do
        task.wait(8)
        ...
        if Config.EggReadyFlag then
            -- Ada egg yang belum diklaim, trigger routine
...
local Tabs = {
    ...
    Chicken   = Window:AddTab({ Title="Chicken",             Icon="egg"           }),
...
local function UpdateClaimLabel() end -- forward declare
...
Tabs.Chicken:AddParagraph({
    Title="Info Sistem Ayam v2.0",
    Content="Logika claim/feed TIDAK diubah dari versi yang works.\nSaat Auto Feed + Auto Claim ON → saat ada telur,\nfarm di-PAUSE dulu, claim+feed, baru resume.\nGunakan kedua toggle ini bersamaan untuk Auto Farm All.",
})
...
Tabs.Chicken:AddParagraph({
    Title="Tips",
    Content="• Aktifkan Auto Feed + Auto Claim bersamaan untuk koordinasi optimal\n• Saat keduanya ON → ada telur = farm di-pause, urus kandang dulu\n• Claim/Feed tidak perlu teleport (sudah works tanpa teleport)\n• Detect kandang dulu sebelum aktifkan toggle",
})
...
task.spawn(function() while true do task.wait(0.1)
    local hum=GetHumanoid()
    if hum and not Config.Freeze then hum.WalkSpeed=Config.WalkSpeed end
end end)
...
task.spawn(function() while true do task.wait(0.1)
    ...
    if hrp and hum and (Config.Freeze or Config.GlobalPause) then
        -- Freeze manual ATAU GlobalPause (chicken routine berjalan)
        -- Anchor player agar physics tidak memindahkan saat teleport ke egg/slot
        hum.WalkSpeed=0; hum.JumpHeight=0; hrp.Anchored=true
    elseif hrp and not Config.Freeze and not Config.GlobalPause then
        hrp.Anchored=false
        if hum then hum.WalkSpeed=Config.WalkSpeed; hum.JumpHeight=7.2 end
    end
end end)
...
Fluent:Notify({
    Title   = "Axorz Hub v2.0",
    Content = "✅ Chicken works + GlobalPause koordinasi!\nAktifkan 4 toggle untuk Auto Farm All.",
    Duration = 5,
})